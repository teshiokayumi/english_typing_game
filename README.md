...
## 先生・教育関係者の方へ：単語リストのカスタマイズについて
このゲームは、デフォルトで私が作成した単語リスト（words.json）を読み込んでいます。
もし、授業の内容に合わせて**オリジナルの単語リスト**を使いたい場合は、以下の手順でカスタマイズできます。

1. このリポジトリをフォーク（コピー）するか、ファイルをダウンロードしてください。
2. `words.json` の中身を、出題したい英単語に書き換えてください。
   例: `["apple", "banana", "cat"]`
3. `main.js` ファイルを開き、書き換えた `words.json` のURLをタイピングトップページにペーストして読み込みボタンを押してください。
